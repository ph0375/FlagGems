...
class DifferentiableLSTMCellBackwardBenchmark(base.Benchmark):
    ...
    def get_input_iter(self, cur_dtype):
        for batch_size, hidden_size, has_bias in self.shapes:
            ...
            grad_cy = torch.randn_like(grad_hy)
            ...
            cx = torch.randn_like(grad_hy)
            cy = torch.randn_like(grad_hy)
            if has_bias:
...
def test_thnn_differentiable_lstm_cell_backward():
    bench = DifferentiableLSTMCellBackwardBenchmark(
        op_name="thnn_differentiable_lstm_cell_backward",
        torch_op=torch.ops.aten._thnn_differentiable_lstm_cell_backward,
        dtypes=DTYPES,
    )
    bench.run()

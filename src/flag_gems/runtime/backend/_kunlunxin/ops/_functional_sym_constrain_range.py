...
@triton.jit
def _sym_constrain_range_copy(x):
    return x
...
def _functional_sym_constrain_range(*args, **kwargs):
    logger.debug("GEMS_KUNLUNXIN _FUNCTIONAL_SYM_CONSTRAIN_RANGE")
    # The functional variant returns a fresh dep_token carrying the data
    # dependency; the actual range constraint on the symint is a trace-time
    ...
    if tensor_arg.is_contiguous() and tensor_arg.numel() > 0:
        if tensor_arg.is_floating_point():
            return _sym_constrain_range_copy(
                tensor_arg, out0=torch.empty_like(tensor_arg)
            )
        return _sym_constrain_range_copy(tensor_arg)
